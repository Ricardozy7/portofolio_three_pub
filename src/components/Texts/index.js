...
function Word({ children, ...props }) {
  ...
  useFrame(({ camera }) => {
    ...
    // Animate font color
    ref.current.material.color.lerp(color.set(hovered ? '#fff' : '#21004B'), 0.1)
  })
...
export default function App() {
  return (
    <Canvas dpr={[1, 2]} camera={{ position: [0, 0, 35], fov: 90 }} style={{ height: 500, width: 500}}>
      <fog attach="fog" args={['#202025', 0, 80]} />
      <Cloud count={8} radius={20} />
      <OrbitControls autoRotate autoRotateSpeed={0.3} zoomSpeed={0.1} />
    </Canvas>
  )
}
